...
var server = net.createServer(function (stream) {
	...
	stream.on('connect', function () {
		...
		stream.write = function () {
			...
			if (!masks[pkt.type]) {
				if(hideCommonPackets && pkt.type != 51 && pkt.type != 4) {
	...
	stream.on('data', function (data) {
		...
		do {
			try {
				...
				if (hideCommonPackets && !helpers.hiddenPackets[pkt.type]) {
...
function cleanupPlayer(clientsession) {
    ...
    var player_idx = world.players.indexOf(clientsession.player);
		
	if(session_idx!=-1) {	
		world.sessions.splice(session_idx, 1);
	}

	if(player_idx!=-1) {	
		world.players.splice(player_idx, 1);
	}
}
