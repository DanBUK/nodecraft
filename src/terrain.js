...
WorldTerrain.prototype.getCellType = function (x, y, z, done_callback) {
	...
	this.getChunk(x, z, function (chunk_data) {
...
WorldTerrain.prototype.recalculateLighting = function (x, z, cb) {
	...
	asyncMap(function (d, cb) {
		me.getChunk((x_i + d[0]) << me.chunk_xz_shift, (z_i + d[1]) << me.chunk_xz_shift, cb);
	}, [
		[-1, -1],
		[0, -1],
		[1, -1],
	...
	function done(data) {
		...
		function chunkFor(x, z) {
			var lx_i = x >> me.chunk_xz_shift,
				lz_i = z >> me.chunk_xz_shift;
			var dx_i = x_i - lx_i,
				dz_i = z_i - lz_i;
				...
				for (var dz = -1; dz <= 1; dz++) {
					for (var dy = -1; dy <= 1; dy++) {
						...
						if (inBounds(x + dx, y + dy, z + dz)) {
							if (isPenetrable(x + dx, y + dy, z + dz) && lightingAt(x + dx, y + dy, z + dz) < light - 1) {
								return false;
							}
						}
					}
		...
		function inBounds(x, y, z) {
			return x >= ((x_i - 1) << me.chunk_xz_shift) && x < ((x_i + 1) << me.chunk_xz_shift) && z >= ((z_i - 1) << me.chunk_xz_shift) && z < ((z_i + 1) << me.chunk_xz_shift) && y >= 0 && y < 128;
		}
